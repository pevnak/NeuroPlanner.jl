using NeuroPlanner.Flux.Optimise: AbstractOptimiser
using NeuroPlanner.Flux.Zygote: withgradient
using Optimisers

debug_data = nothing

function train(loss, model, rule::AbstractRule, prepare_minibatch, max_steps; reset_fval = 1000, verbose = true, stop_fval=typemin(Float64), logger = nothing, trn_data = [], debug = false)
	fval, n = 0.0, 0
	last_fval = nothing
	...
	for i in 1:max_steps
		global debug_data
		d = prepare_minibatch()
		fᵢ, ∇model = withgradient(model -> loss(model, d), model)
		∇model = only(∇model)
		if debug 
			#@show fᵢ
			global debug_data
			debug_data = deepcopy((model, d))
			isinvalid(∇model) && error("gradient is isinvalid")
		end
		!isfinite(fᵢ) && error("Loss is $fᵢ on data item")
		fval += fᵢ
		n += 1
