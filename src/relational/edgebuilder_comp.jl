mutable struct EdgeBuilderComp{T<:Integer,I<:Integer}
    indices::Vector{T}
    counts::Vector{I}
    arity::Int
    num_observations::Int
    offset::Int
    function EdgeBuilderComp(indices::Vector{T}, counts::Vector{I}, arity::Int, num_observations::Int, offset::Int) where {T,I}
        ...
        new{T,I}(indices, counts, arity, num_observations, offset)
    end
end
...
mutable struct EdgeBuilderCompMat{T<:Integer,I<:Integer}
    indices::Matrix{T}
    counts::Vector{I}
    arity::Int
    num_observations::Int
    offset::Int
    function EdgeBuilderCompMat(indices::Matrix{T}, counts::Vector{I}, arity::Int, num_observations::Int, offset::Int) where {T,I}
        arity > 0 || error("Can create only edge of positive arity.")
        offset == 0 || error("Offset needs to start at 0.")
        length(indices) == arity * num_observations || error("Length of indices must be equal to `arity * num_observations`.")
        new{T,I}(indices, counts, arity, num_observations, offset)
...
function EdgeBuilderCompMat(::Val{arity}, num_observations::Int, numobs::Int) where {arity}
    indices = Matrix{Int}(undef, arity, num_observations)
    counts = fill(0, numobs)
    offset = 0
    EdgeBuilderCompMat(indices, counts, arity, num_observations, offset)
end


function EdgeBuilderCompMat(arity::Int, num_observations::Int, numobs::Int)
    indices = Matrix{Int}(undef, arity, num_observations)
    counts = fill(0, numobs)
    offset = 0
    EdgeBuilderCompMat(indices, counts, arity, num_observations, offset)
...
function Base.push!(eb::EdgeBuilderCompMat, vertices::NTuple{N,I}) where {N,I<:Integer}
    eb.offset += 1
    _mapenumerate_tuple(vertices) do i, vᵢ
        eb.counts[vᵢ] += 1
...
function construct(eb::EdgeBuilderCompMat, input_sym::Symbol)
