...
"""
@inline function _mapenumerate_tuple(f::Function, xs::Tuple)
    isempty(xs) && return(xs)
    tuple(f(1, first(xs)), _mapenumerate_tuple(f, 2, Base.tail(xs))...)
end

@inline function _mapenumerate_tuple(f::Function, i::Int,  xs::Tuple)
    isempty(xs) && return(xs)
...
function _inlined_search(s, i::Int,  xs::Tuple)
    isempty(xs) && return(-1)
    s == first(xs) && return(i)
    return(_inlined_search(s, i+1, Base.tail(xs)))
end
