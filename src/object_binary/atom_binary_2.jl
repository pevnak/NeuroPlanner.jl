...
"""
struct AtomBinary2{DO,MP,D,S,G}
    domain::DO
    max_arity::Int
    ...
    function AtomBinary2(domain::DO, max_arity, constmap, actionmap, model_params::MP, obj2id::D, init::S, goal::G) where {DO,D,MP<:NamedTuple,S,G}
...
AtomBinary2NoGoal{DO,MP,D} = AtomBinary2{DO,MP,D,Nothing,Nothing} where {DO,MP,D}
AtomBinary2Start{DO,MP,D,S} = AtomBinary2{DO,MP,D,S,Nothing} where {DO,MP,D,S<:AbstractVector{<:Julog.Term}}
AtomBinary2Goal{DO,MP,D,G} = AtomBinary2{DO,MP,D,Nothing,G} where {DO,MP,D,G<:AbstractVector{<:Julog.Term}}

function AtomBinary2(domain; message_passes=2, residual=:linear, kwargs...)
    ...
    AtomBinary2(domain, max_arity, constmap, actionmap, model_params, nothing, nothing, nothing)
end

isspecialized(ex::AtomBinary2) = ex.obj2id !== nothing
hasgoal(ex::AtomBinary2) = ex.init_state !== nothing || ex.goal_state !== nothing


function AtomBinary2(domain, problem; embed_goal=true, kwargs...)
    ex = AtomBinary2(domain; kwargs...)
    ex = specialize(ex, problem)
    embed_goal ? add_goalstate(ex, problem) : ex
end

function Base.show(io::IO, ex::AtomBinary2)
    if !isspecialized(ex)
...
"""
specialize(ex::AtomBinary2{<:Nothing,<:Nothing}, problem)
...
extractor.
"""
function specialize(ex::AtomBinary2, problem)
    obj2id = Dict(v.name => i for (i, v) in enumerate(problem.objects))
    for k in keys(ex.constmap)
        obj2id[k] = length(obj2id) + 1
    end
    AtomBinary2(ex.domain, ex.max_arity, ex.constmap, ex.actionmap, ex.model_params, obj2id, nothing, nothing)
end

function (ex::AtomBinary2)(state::GenericState)
    encode_state(ex, state)
end

function encode_state(ex::AtomBinary2, state::GenericState, prefix=nothing)
...
"""
    nunary_predicates(ex::AtomBinary2, atoms, gii)
    ...
    last row.
"""
function unary_predicates(ex::AtomBinary2, atoms, gii)
    # encode constants
...
function group_facts(ex::AtomBinary2, atoms::Vector{Julog.Term})
...
function encode_edges(ex::AtomBinary2, atoms::Vector{Julog.Term}, kid::Symbol)
...
@inline function _type_of_edge(ex::AtomBinary2, i, j)
    ex.max_arity*(i-1) + j
end


function add_goalstate(ex::AtomBinary2, problem, goal=goalstate(ex.domain, problem))
    ex = isspecialized(ex) ? ex : specialize(ex, problem)
    @set ex.goal_state = collect(get_facts(goal))
end

function add_initstate(ex::AtomBinary2, problem, start=initstate(ex.domain, problem))
    ex = isspecialized(ex) ? ex : specialize(ex, problem)
...
"""
function add_goalatoms(ex::AtomBinary2Start, atoms)
    gid = 1:length(atoms)
    vcat(atoms, ex.init_state), gid
end

function add_goalatoms(ex::AtomBinary2Goal, atoms)
    gid = 1:length(ex.goal_state)
    vcat(ex.goal_state, atoms), gid
end

function add_goalatoms(ex::AtomBinary2NoGoal, atoms)
    gid = 0:-1
    atoms, gid
end
