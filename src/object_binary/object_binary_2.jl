"""
struct ObjectBinary2{DO,P,EB,MP,D,S,G}
    domain::DO
    predicates::P
    edgebuilder::EB
...
`ObjectBinary2FE` --- representing multiple edges as edges with features
`ObjectBinary2FENA` --- representing multiple edges as edges with features, but do not aggregate multiple edges
`ObjectBinary2ME` --- representing edges as proper multi-graph
"""
...
struct ObjectBinary2{DO,P,EB,MP,D,S,G}
    ...
    init_state::S
    goal_state::G
    function ObjectBinary2(domain::DO, predicates::P, edgebuilder::EB, objtype2id::Dict{Symbol,Int64}, 
...
ObjectBinary2NoGoal{DO,P,edgebuilder,MP} = ObjectBinary2{DO,P,edgebuilder,MP,D,Nothing,Nothing} where {DO,P,edgebuilder,MP,D}
ObjectBinary2Start{DO,P,edgebuilder,MP,D,S} = ObjectBinary2{DO,P,edgebuilder,MP,S,D,Nothing} where {DO,P,edgebuilder,MP,D,S<:Vector}
ObjectBinary2Goal{DO,P,edgebuilder,MP,D,G} = ObjectBinary2{DO,P,edgebuilder,MP,D,Nothing,G} where {DO,P,edgebuilder,MP,D,G<:Vector}

predicates_of_length(domain::GenericDomain, l) = tuple(filter(k -> length(domain.predicates[k].args) == l, keys(domain.predicates))...)

function ObjectBinary2(domain; message_passes=2, residual=:linear, edgebuilder = FeaturedEdgeBuilder, kwargs...)
    ...
    ObjectBinary2(domain, pifo, edgebuilder, objtype2id, constmap, model_params, nothing, nothing, nothing)
end

ObjectBinary2FE(domain;kwargs...) = ObjectBinary2(domain;edgebuilder = FeaturedEdgeBuilder, kwargs...)
ObjectBinary2FENA(domain;kwargs...) = ObjectBinary2(domain;edgebuilder = FeaturedEdgeBuilderNA, kwargs...)
ObjectBinary2ME(domain;kwargs...) = ObjectBinary2(domain;edgebuilder = MultiEdgeBuilder, kwargs...)

isspecialized(ex::ObjectBinary2) = ex.obj2id !== nothing
hasgoal(ex::ObjectBinary2) = ex.init_state !== nothing || ex.goal_state !== nothing

function ObjectBinary2(domain, problem; embed_goal=true, kwargs...)
    ex = ObjectBinary2(domain; kwargs...)
    ex = specialize(ex, problem)
    embed_goal ? add_goalstate(ex, problem) : ex
end

function Base.show(io::IO, ex::ObjectBinary2)
    if !isspecialized(ex)
...
"""
specialize(ex::ObjectBinary2{<:Nothing,<:Nothing}, problem)
...
extractor.
"""
function specialize(ex::ObjectBinary2, problem)
    obj2id = Dict(v.name => i for (i, v) in enumerate(problem.objects))
    for k in keys(ex.constmap)
        obj2id[k] = length(obj2id) + 1
    end
    ex = ObjectBinary2(ex.domain, ex.predicates, ex.edgebuilder, ex.objtype2id, ex.constmap, ex.model_params, obj2id, nothing, nothing)
end


function intstates(ex::ObjectBinary2, state::GenericState)
    facts = collect(PDDL.get_facts(state))
    intstates(ex.domain, ex.obj2id, ex.predicates, facts)
end

function (ex::ObjectBinary2)(state::GenericState)
...
function encode_state(ex::ObjectBinary2, state::GenericState, grouped_facts)
...
"""
nunary_predicates(ex::ObjectBinary2, state)
...
"""
function nunary_predicates(ex::ObjectBinary2, state, grouped_facts)
...
function multi_predicates(ex::ObjectBinary2, kid::Symbol, grouped_facts, prefix=nothing)
...
"""
encode_predicates(ex::ObjectBinary2, pname::Symbol, preds, kid::Symbol)

Encodes predicates for an ObjectBinary2 instance.

Arguments:
- ex::ObjectBinary2: ObjectBinary2 instance
...
This function encodes predicates for an ObjectBinary2 instance using the given predicate name, predicates, and key ID.
"""
function encode_predicates!(eb::EB, ex::ObjectBinary2, preds::Vector{<:IntState{N,<:Integer}}) where {N, EB<:Union{FeaturedEdgeBuilder,MultiEdgeBuilder}}
    for p in preds
        for i in 1:N
            oᵢ = p.args[i]
            for j in i+1:N
...
function add_initstate(ex::ObjectBinary2, problem, start=initstate(ex.domain, problem))
...
function add_goalstate(ex::ObjectBinary2, problem, goal=goalstate(ex.domain, problem))
    ex = isspecialized(ex) ? ex : specialize(ex, problem)
...
function addgoal(ex::ObjectBinary2Start, kb)
    return (merge_states(ex.init_state, kb))
end

function addgoal(ex::ObjectBinary2Goal, kb)
    return (merge_states(kb, ex.goal_state))
end

function addgoal(ex::ObjectBinary2NoGoal, kb)
    return (kb)
end
